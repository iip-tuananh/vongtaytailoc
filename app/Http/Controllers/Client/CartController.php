...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $data['cart'] = session()->get('cart',[]);
        $html = view('cart.header-mini-cart', $data)->render();
        $html1 = view('cart.header-mini-count-cart', $data)->render();
        $html2 = view('cart.popup-cart', $data)->render();
    ...
    public function update(Request $request)
    {
        if($request->id && $request->quantity){
            ...
            $data['cart'] = session()->get('cart',[]);
            $html = view('cart.header-mini-cart', $data)->render();
            $html1 = view('cart.update-list-cart', $data)->render();
            return response()->json([
                'html'=>$html,
                'html1'=>$html1,
            ]);
    ...
    public function remove(Request $request)
    {
        if($request->id) {
            ...
            $html = view('cart.header-mini-cart', $data)->render();
            $html1 = view('cart.header-mini-count-cart', $data)->render();
            $html2 = view('cart.update-list-cart', $data)->render();
            return response()->json([
                'html'=>$html,
                'html1'=>$html1,
                'html2'=>$html2,
